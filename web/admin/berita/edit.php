...
$res_berita = mysqli_query($mysqli, "SELECT * FROM tb_berita WHERE id=$id");

while ($berita = mysqli_fetch_array($res_berita)) {
    $row_judul_berita = $berita['judul_berita'];
    $row_content_berita = $berita['content_berita'];
    $row_kategori = $berita['id_kategori'];
}
?>
<?php
...
if (isset($_POST['update'])) {
    ...
    $tipe = $gambar['type'];
    $size = $gambar['size'];
    if ($data = mysqli_fetch_array($res_berita)) {
            $image = $data['gambar'];
    ...
    if (is_uploaded_file($gambar['tmp_name'])) {
        if (!in_array(($tipe), $ext)) {
            ...
            if (move_uploaded_file($gambar['tmp_name'], $dir . $newName)) {
                $result = mysqli_query($mysqli, "UPDATE tb_berita SET cover='$newName',judul_berita='$judul_berita',
                content_berita='$content_berita',id_kategori='$kategori',user_id='$user_id',created_time='$created_time'
                WHERE id=$id");
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="post"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" value="<?= $row_judul_berita ?>"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea type="text" class="form-control" name="content_berita"
                                                required><?= $row_content_berita ?></textarea>
                                        </div>
                                        <?php
